...
namespace Application.Catalogs.CatalogTypes.Query
{
 ...
 .Include(p=>p.CatalogItems.Where(p=>p.Importance>0 && p.IsActive==true).OrderByDescending(p=>p.Importance).Take(15)).ThenInclude(p=>p.CatalogItemImages)
 .Include(p => p.CatalogItems.Where(p => p.Importance > 0 && p.IsActive == true).OrderByDescending(p => p.Importance).Take(15)).ThenInclude(p => p.Discounts)
 .Include(p => p.CatalogItems.Where(p => p.Importance > 0 && p.IsActive == true).OrderByDescending(p => p.Importance).Take(15)).ThenInclude(p => p.CatologCar)
 .Include(p => p.CatalogItems.Where(p => p.Importance > 0 && p.IsActive == true).OrderByDescending(p => p.Importance).Take(15)).ThenInclude(p => p.CatalogBrand)
  .ToList();
